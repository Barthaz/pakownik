...
export function useSharedListController(shareId: string) {
  const [list, setList] = useState<import('@/models/types').SharedList | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await shareService.getShared(shareId);
      setList(data);
    } catch (err) {
      setError((err as Error).message);
    } finally {
  ...
  const canCheck = list?.sharePermission !== 'readonly';
  const canEdit = list?.sharePermission === 'full_edit';

  const togglePacked = useCallback(
    async (itemId: string) => {
      if (!list || !canCheck) return;
      const { shareService } = await import('@/services/shareService');
      const updated = await shareService.togglePacked(shareId, itemId);
      setList((prev) =>
        prev
          ? { ...prev, items: prev.items.map((i) => (i.id === itemId ? updated : i)) }
          : null,
  ...
  const addItem = useCallback(
    async (data: { category: string; name: string; quantity: number }) => {
      ...
      const { shareService } = await import('@/services/shareService');
      const item = await shareService.createItem(shareId, data);
      setList((prev) => (prev ? { ...prev, items: [...prev.items, item] } : null));
    },
    [list, shareId, canEdit],
  );
  ...
  const deleteItem = useCallback(
    async (itemId: string) => {
      if (!list || !canEdit) return;
      const { shareService } = await import('@/services/shareService');
      await shareService.deleteItem(shareId, itemId);
      setList((prev) =>
        prev ? { ...prev, items: prev.items.filter((i) => i.id !== itemId) } : null,
      );
    },
  ...
  const updateItem = useCallback(
    async (itemId: string, data: { name?: string; quantity?: number }) => {
      if (!list || !canEdit) return;
      const { shareService } = await import('@/services/shareService');
      const updated = await shareService.updateItem(shareId, itemId, data);
      setList((prev) =>
        prev
          ? { ...prev, items: prev.items.map((i) => (i.id === itemId ? updated : i)) }
          : null,
